...
class ElasticNet:
    """Elastic Net regression represents the combined L1 and L2 regualarized linear model for
    ...
    :param lmbd1: The regularization parameter for L1 norm
    :param lmbd2: The regularization parameter for L2 norm

    """

    def __init__(self, n, max_iter=500, lmbd1=1e-3, lmbd2=1e-3, optimizer=None):
        self.N = n
    ...
    def fit(self):
        ...
        frac = self.lmbd1 / self.optimizer.rho
    ...
    def objective_x(self, a, b, x, z, u, rho):
        return self.loss_fn(a, b, x) + rho / 2 * self.regularizer_x(x, z, u) + self.lmbd2 * cp.norm(x)**2
...
def main():
    ...
    elnet = ElasticNet(n=n, max_iter=500, lmbd1=1e-3, lmbd2=1e-3, optimizer=optimizer)
    optimizer.objective_fn = elnet.objective_x
    ...
    print(elnet.predict(np.random.rand(50, 50)))
