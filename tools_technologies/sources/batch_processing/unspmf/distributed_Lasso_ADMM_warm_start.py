"""
ADMM Lasso using warm start

@Author: Faculty of Sciences, University of Novi Sad, Serbia
...
def admm_lasso(N = 10, max_iter = 500, lmbd = 1e-3, rho = 1, abstol = 1e-4, reltol = 1e-2):
    ...
    for i in range(max_iter):
        #update x
        x = list(map(functools.partial(x_update, z,  rho), data_chunk, target_chunk, x, u))
        x = compss_wait_on(x)
